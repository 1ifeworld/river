...
ponder.on("LogicTransmitterMerkleAdmin:AccountRolesSet", async ({event, context}) => {
    ...
    const existingAdmin = await LogicTransmitterMerkleAdmin.findUnique({id: press});
  
    if (existingAdmin) {
      const newAccounts = existingAdmin.accounts ? existingAdmin.accounts.concat(accounts) : accounts;
      const newRoles = existingAdmin.roles ? existingAdmin.roles.concat(roles) : roles;
