...
import { isValidSchemaRegistration } from "./offchainSchema/idRegistry";
import { decodeNodeRegistrationData } from "./offchainSchema/nodeRegistry";
...
ponder.on("NodeRegistry:RegisterSchema", async ({ event, context }) => {
  ...
  if (!isValidSchemaRegistration({sender: sender, schema: schema, data: data})) return
...
ponder.on("NodeRegistry:RegisterNode", async ({ event, context }) => {
  ...
  const { sender, nodeId, data } = event.params;
    ...
    const {userId, schema, regType, regBody} = decodeNodeRegistrationData({ data: data })
