...
export function ContentWrapper({ item, className, children }: ContentWrapperProps) {
    ...
    const currentReferenceIndex = item.channel?.references.findIndex(ref => ref.id === item.id)
    ...
    if (baseChannelReferences) {
        nextReference = currentReferenceIndex == baseChannelReferences.length - 1 
            ? null 
            : item.channel?.references[currentReferenceIndex as number + 1].id
        prevReference = currentReferenceIndex == 0 
            ? null 
            : item.channel?.references[currentReferenceIndex as number - 1].id
    }
