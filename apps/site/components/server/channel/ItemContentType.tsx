...
export async function ItemContentType({ item }: { item: Item }) {
  ...
  try {
    ...
    console.log("Metadata response for URI", publicationUri, ":", metadataResponse);

    if (!metadataResponse || !metadataResponse.data || !metadataResponse.data[publicationUri]) {
      console.error("No metadata found for URI:", publicationUri);
      return <Typography className="text-primary-foreground">Unknown Type</Typography>;
    }
    ...
    if (!itemMetadata.content_type) {
      console.error("No content_type found in metadata for URI:", publicationUri);
      return <Typography className="text-primary-foreground">No Content Type</Typography>;
    }
